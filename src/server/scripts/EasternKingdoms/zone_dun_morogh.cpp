...
class spell_extinguish_fire : public SpellScript
{
    ...
    {
        // recheck for targets.remove to nearest creature of entry
        targets.clear();
        targets.push_back(GetCaster()->FindNearestCreature(NPC_BURNING_BOMBER_CREDIT, 9.0f, true));
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_extinguish_fire::FilterTargets, EFFECT_0, TARGET_UNIT_CONE_ENTRY);
